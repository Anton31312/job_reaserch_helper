...
Для запуска программы необходимо создать в корне проекта файл *.env*. Прописать в этом файле переменную SUPER_JOB_TOKEN =... со значением соответствующим токену для API сервиса SuperJob или установить данную переменную как переменную окружения.

Произвести установку требуемых библиотек. Ввести команду в терминале *$ poetry install*

Точка входа в приложение - файл *main.py* расположенный в корне проекта.

Для изменения формата хранения данных следует переопределить класс с помощью которого создается переменная save на строке 15 в файле main: 
